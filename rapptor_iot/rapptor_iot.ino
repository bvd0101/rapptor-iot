...
// --- MUSIC NOTES DEFINITION (PITCHES) ---
#define NOTE_C4  262
#define NOTE_E5  659
#define NOTE_G4  392
#define NOTE_G5  784
...
int melody[] = {
  NOTE_E5, NOTE_E5, REST, NOTE_E5, REST, NOTE_C5, NOTE_E5, NOTE_G5, REST, REST, NOTE_G4
};
int tempo[] = {
  8, 8, 8, 8, 8, 8, 8, 4, 8, 8, 4
};
...
void manageLed() {
  if (WiFi.status() == WL_CONNECTED) {
    int rssi = WiFi.RSSI();
    if (rssi >= -55)      setLedColor(0, 255, 0);   // Green
    else if (rssi >= -65) setLedColor(255, 255, 0); // Yellow (Red + Green)
    else if (rssi >= -75) setLedColor(255, 100, 0); // Orange
    else                  setLedColor(255, 0, 0);   // Red
  } else {
    setLedColor(0, 0, 255); // Blue when disconnected
  }
}
